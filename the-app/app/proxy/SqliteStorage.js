...
Ext.define('Sqlite.data.proxy.SqliteStorage', {
	...
	truncate: function (tablename) {
		...
		me.transactionDB(me.getDb(), [function (tx) {
			me.executeSql(tx, sql, [], Ext.emptyFn, Ext.emptyFn);
	...
	read: function (operation, callback, scope) {
		...
		me.transactionDB(me.getDb(), [function (tx) {
			me.executeSql(tx, sql, [], onSuccess, onError);
		}], Ext.emptyFn, Ext.emptyFn);
	},
	...
	setTable: function () {
		var me = this,
				...
				createTable = function (tx) {
					me.executeSql(tx, 'CREATE TABLE IF NOT EXISTS ' +
				...
				checkDataExists = function (tx) {
					...
					me.executeSql(tx, 'SELECT * FROM ' + me.getDbConfig().tablename + ' LIMIT 1',
							[], function (tx, result) {

								if (result.rows.length == 0) {
									me.executeSql(tx, 'DROP TABLE IF EXISTS ' + me.getDbConfig().tablename, [],
											createTable, onError);
								} else {
				...
				checkExistingSchema = function (tx, data) {
					...
					Ext.each(Ext.Array.difference(neededFieldNames, existingFieldNames),
							function (addField) {
								...
								me.executeSql(tx, 'ALTER TABLE ' + me.getDbConfig().tablename +
										...
										function (tx) {
											...
											me.executeSql(tx, 'UPDATE ' + me.getDbConfig().tablename +
		...
		me.transactionDB(me.getDb(), [function (tx) {
			me.executeSql(tx, 'SELECT sql FROM sqlite_master WHERE name=?',
    ...
    dropTable: function(tablename) {
       ...
       me.getDb().transaction(function(tx) {
        ...
        me.executeSql(tx, tablesql,[],onSuccess,onError);
       });
    },
